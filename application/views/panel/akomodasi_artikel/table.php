	...
	<div class="col-md-12">
		<div class="portlet light">
            ...
            <div class="portlet-body">
            	<div class="row">         			
                    <div class="col-md-6">
	                    <a href="<?=site_url('panel/akomodasi_artikel/index/' . $uri_kategori)?>" class="btn blue">
	                        <i class="fa fa-history hidden-xs"></i> Kembali 
	                    </a>
	                    <a href="<?=site_url('panel/akomodasi_artikel/form/' . $uri_kategori . '/' . $uri_objek)?>" class="btn green">
            		...
            		<div class="col-md-6">
            			<h4>Cari Data</h4>
		           		<form method="GET" action="<?=site_url('panel/akomodasi_artikel/index/' . $uri_kategori . '/' . $uri_objek)?>">
		           			<div class="row">
							...
							<?php foreach($data as $key => $c): ?>
								<tr>
									<td class="text-center">
										<a href="<?=site_url('panel/akomodasi_artikel/detail/' . $uri_kategori . '/' . $uri_objek . '/' . $c->artikel_id)?>" class="btn grey btn-xs" title="Lihat Artikel">
											<i class="fa fa-list"></i>
										</a>
										<?php if($login_level == 'Administrator' || $login_uid == $c->user_id){ ?> 
											<a href="<?=site_url('panel/akomodasi_artikel/form/' . $uri_kategori . '/' . $uri_objek . '/' . $c->artikel_id)?>" class="btn blue btn-xs" title="Perbaharui / Update">
									...
									<td>
										<a href="<?=site_url('panel/akomodasi_artikel/detail/' . $uri_kategori . '/' . $uri_objek . '/' . $c->artikel_id)?>" class="btn green btn-xs" title="Silahkan Periksa terlebih dahulu sebelum mengganti status">
											<?=$c->status?>
										</a>
									</td>	
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-footer">
            	<a href="<?=site_url('panel/akomodasi_artikel')?>" id="btn-yes" class="btn red">
...
<script type="text/javascript">
	...
	{
		$('#modal-hapus #btn-yes').attr({'href' : '<?=site_url('panel/akomodasi_artikel/hapus/' . $uri_kategori . '/' . $uri_objek)?>/' + id});
		$('#modal-hapus').modal();
	}
</script>