	...
	<div class="col-md-12">
		<div class="portlet light">
            ...
            <div class="portlet-body">
            	<div class="row">         			
                    <div class="col-md-3">
	                    <a href="<?=site_url('panel/objek/form')?>" class="btn green btn-sm btn-block">
	                        <i class="fa fa-plus hidden-xs "></i> Tambah Data
	                    </a>
	                    <?php if($login_level == 'Administrator'){ ?>
		                    <hr style="margin: 2px 0px;" />
		                    <a href="<?=site_url('panel/objek/export_kml')?>" class="btn green btn-sm btn-block">
		                        <i class="fa fa-arrow-circle-up hidden-xs"></i> Export KML 
		                    </a>
	                    	<hr style="margin: 2px 0px;" />
		                    <a href="<?=site_url('panel/objek/import_kml')?>" class="btn green btn-sm btn-block">
		                        <i class="fa fa-arrow-circle-down hidden-xs"></i> Import KML 
		                    </a>
		                <?php } ?>
            		...
            		<div class="col-md-9">
            			<h4>Cari Data</h4>
		           		<form method="GET" action="<?=site_url('panel/objek')?>">
		           			<div class="row">
								...
								<tr>
									<td class="text-center">
										<div class="btn-group btn-group-justified" style="width:100px">
											<a href="<?=site_url('panel/objek/detail/' . $c->objek_id)?>" class="btn grey btn-xs" title="Lihat Objek">
												<i class="fa fa-list"></i>
											</a>
											<?php if($c->status != 'Moderasi'){ ?>
												<a href="<?=site_url('panel/objek/form/' . $c->objek_id)?>" class="btn blue btn-xs" title="Perbaharui / Update">
									...
									<td>
										<a href="<?=site_url('panel/objek/detail/' . $c->objek_id)?>" class="btn green btn-xs" title="Silahkan Periksa terlebih dahulu sebelum mengganti status">
											<?=$c->status?>
										</a>
									</td>	
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-footer">
            	<a href="<?=site_url('panel/objek')?>" id="btn-yes" class="btn red">
...
<script type="text/javascript">
	...
	{
		$('#modal-hapus #btn-yes').attr({'href' : '<?=site_url('panel/objek/hapus')?>/' + id});
		$('#modal-hapus').modal();
	}
</script>