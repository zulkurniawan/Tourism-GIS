	...
	<div class="col-md-12">
		<div class="portlet light bordered">
            <div class="portlet-title">
				...
				<div class="actions">
				    <a class="btn blue" href="<?=site_url('panel/objek')?>">
				    	<i class="fa fa-history hidden-xs"></i> Kembali
				    </a>
				    <?php if($login_level == 'Administrator' || $login_uid == $data->user_id){ ?>
						<a href="<?=site_url('panel/objek/form/' . $data->objek_id)?>" class="btn blue btn-xs" title="Perbaharui / Update">
            	...
            	<div class="row">
            		<div class="col-md-12">
						<div class="note note-info">
            				<p>Kontributor : <strong><?=$data->nama_kontributor?></strong>, 
            				   Terakhir Update : <strong><?=date('H:i:s d-m-Y', strtotime($data->terakhir_update))?></strong></p>
    					</div>
            		</div>
            	</div>
            	...
            	<div class="row">
	            	<div class="col-md-5">
						...
						<div class="portlet light bordered">
				            ...
				            <div class="portlet-body">
				            	<ul class="list-group">
				            		<li class="list-group-item">Tiket : <br/><?=nl2br($data->info_tiket)?></li>
				            		<li class="list-group-item">Tempat Ibadah : <?=$data->info_tempat_ibadah?></li>
				            		<li class="list-group-item">Penginapan : <?=$data->info_penginapan?></li>
				            		<li class="list-group-item">Toilet : <?=$data->info_toilet?></li>
				            		<li class="list-group-item">Akses Jalan : <?=$data->info_akses_jalan?></li>
				            	</ul>
				        	</div>
				        </div>
						<div class="portlet light bordered">
						...
						<div class="portlet light bordered">
				            <div class="portlet-title">
				                <div class="caption">
				                    ...
				                    <span class="caption-subject bold uppercase">Peta</span>
				                </div>
				            </div>
				            <div class="portlet-body">
						...
						<div class="portlet light bordered">
				            <div class="portlet-title">
				                <div class="caption">
				                    ...
				                    <span class="caption-subject bold uppercase">Street View</span>
				                </div>
				            </div>
				            <div class="portlet-body">
				            	<div id="street_view" style="width: 100%; height: 400px;"></div>
				            </div>
				        </div>
	            	</div>
	            </div>
...
<script>
    ...
    function initMap() 
    {
		...
		<?php if(empty($data->street_view_url)){ ?>
			...
			streetViewService.getPanoramaByLocation(myLatLng, STREETVIEW_MAX_DISTANCE, function (streetViewPanoramaData, status) {
			    if (status === google.maps.StreetViewStatus.OK) {
			    	...
			    	$('#street_view').attr({'style' : 'background-color: #000; color: #FFFFFF; text-align: center; height: 400px; padding-top: 100px;'});
			    	$('#street_view').html('<h4>Street view tidak tersedia.</h4>');
			    }
			});
		<?php } else { ?>
			$('#street_view').html('<iframe src="<?=$data->street_view_url?>" width="100%" height="400px" frameborder="0" style="border:0" allowfullscreen></iframe>');
		<?php } ?>
  	}
</script>
...
<div class="modal fade" id="modal-status" tabindex="-1" role="basic" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
        	<form class="form-horizontal" action="<?=site_url('panel/objek/set_status/' . $data->objek_id)?>" method="POST">
