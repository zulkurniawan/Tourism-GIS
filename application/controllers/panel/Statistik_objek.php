...
class Statistik_objek extends CI_Controller
{
	function __construct()
	{
		...
		$this->page_active = 'statistik_objek';
	}
	...
	public function index()
	{
		...
		$param['data']	= $this->statistik_objek_model->get_data($filter)->result();

		unset($filter['limit']);
		unset($filter['offset']);
		$total_rows 			= $this->statistik_objek_model->get_data($filter)->num_rows();
		$param['pagination']	= paging('panel/statistik_objek/index', $total_rows, $limit, $uri_segment);

		$param['populer']		= $this->statistik_objek_model->get_data(array('limit' => 10))->result();

		$param['opt_kategori']	= $this->objek_model->get_opt_kategori(true);
		$param['main_content']	= 'panel/statistik_objek/main';
	...
	public function detail($objek_id = '')
	{
		...
		$param['data_view_objek']			= $this->statistik_objek_model->get_data_view_objek($objek_id, $param);
		$param['data_view_unik_visitor']	= $this->statistik_objek_model->get_data_unik_visitor($objek_id, $param);
		$param['data_objek']				= $this->statistik_objek_model->get_data_row($objek_id);
		// echo '<pre>';
		// print_r($param['data_objek']);
		// exit;

		$param['main_content']	= 'panel/statistik_objek/detail';
		$param['page_active'] 	= $this->page_active;
		$this->templates->load('templates_panel', $param);		
	}
}